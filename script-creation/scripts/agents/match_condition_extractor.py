"""步骤 2a — 依据《度小满-达人类型基础标准（终版）》判定达人类型。

输入：达人风格 JSON（influencer-style-analysis 输出）
输出：类型判定 JSON（primary_type 一级类型、secondary_type 二级类型）

标准文档：https://kwza968lz1u.feishu.cn/docx/QDPOdmP6XoNiqpxmbl6cwNianCg
...
SPEC = AgentSpec(
    ...
    instructions="""你是达人类型判定专家。任务：依据《度小满-达人类型基础标准（终版）》，判定达人属于哪个一级类型和二级类型。
...
### 生活
- 民生：选题全部来自"普通人正在经历、但自己说不清楚"的事，不追专业议程（不讲货币政策本身），只抓大众体感（钱变难赚了、婚不敢结了、学历不值钱了）；落点永远是"你个人怎么办"，宏观事件只当引子，三句话内必然折到个人决策；话题自带阶层站位（站普通打工人）；人设=平视过来人，有可验证的平民经历叙事，无权威身份包装
- 人文杂谈：选题源是"人活不明白"的困惑（被拿捏、被套话、内耗、看不透人情世故），钱只是偶发话题无连续主线；落点是处世方法论或认知消遣（非金钱决策）；站位无阶层对抗性（说书人或阅历权威）；关键检验：观众对该账号无"金钱托付感"
...
5. 参考锚点：财经-泛财经（商业小纸条、直言财经、达哥有点味）、财经-高价值（小张小张吃饭用缸、许戈、大哲）、财经-小微企业主（乡村大宁、包工头才哥）、生活-民生（萌叔的小酒馆、朴素之道、刘雯）、生活-人文杂谈（涵浪、万能春秋、赵大湿）、三农-三农美食（董姐夫美食、周甄甄）、三农-三农建造（建造哥户外生活、文子大大、手工耿）、剧情-常规剧情（帅大斌的一天、三根葱）、剧情-剧情搞笑（太和小夫妻、猪肉佬）
