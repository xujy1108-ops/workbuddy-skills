---
name: hotspot
description: "热点抓取工作流（多品牌配置化）。从抖音/小红书/快手/微博/B站/政策解读/抖音头部达人采集全网热点，用 AI 判断是否符合本品牌时事政策热点（金融经济、民生经济、和钱相关、普罗大众相关），并为符合的热点选语义锚点、数转折跳数、按跳数定植入方式（直接阐述/隐喻植入/仅蹭热度），写入飞书热点素材表；支持手动精筛——对入表热点做抖音搜索点赞定级（优秀/良好/一般/劣质）并回填评分。品牌私有内容全部在 config/<brand>/，用 --brand 切换。触发词：热点抓取、品牌热点、跑热点、精筛第N行、热点筛选、热点入表。"
agent_created: true
---
...
> ⚠️ 目录名 `hotspot`（原名 `duxiaoman-hotspot`，2026-09-23 去品牌化改名），本身是通用的多品牌热点工作流（脚本无品牌硬编码）。
