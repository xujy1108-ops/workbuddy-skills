...
async function main() {
  ...
  log(`\n🔹 待 AI 判断热点 ${candidates.length} 条`);

  // Step 2: AI 判断（串行，控制 RPS）
  const judged = [];
  for (let i = 0; i < candidates.length; i++) {
    const it = candidates[i];
    process.stderr.write(`   [${i + 1}/${candidates.length}] ${it.topic?.substring(0, 30)} ... `);
    try {
      const j = await judgeHotspot(it, opts.debug);
      process.stderr.write(j.fit ? '✓符合\n' : '✗不符\n');
      judged.push({ item: it, judge: j });
    } catch (e) {
      process.stderr.write(`❌ ${e.message.substring(0, 80)}\n`);
      judged.push({ item: it, judge: { fit: false, reason: e.message, strategy: '' } });
    }
  }
