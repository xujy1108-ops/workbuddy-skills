...
| `HOTSPOT_BASE_TOKEN` | 热点素材表 Base Token（探索素材来源） | `STMrbQgqma35dksI3WsclJlNnlc` |
...
2. **生成关键词** — DeepSeek 按动态配额生成 4 个抖音搜索关键词：①策略直搜（运行时动态拉取内容策略文档，把 S/A 级策略翻译成用户会搜的词，24h 缓存+兜底清单）②策略衍生（抽象策略核心钩子公式，换人群/职业实例化）③开放探索×2（2026-09-15 反内循环改造：优先注入热点素材表 7 天内新鲜热点作为探索素材——把热点牵出的用钱场景翻译成搜索词；热点表读取失败/无新鲜热点时降级为 AI 自由衍生）。**加强模式**：连续 2 轮零新策略产出（无新建方向且无追加打法）时自动切换为 直搜1 + 探索3、策略衍生暂停，打断"关键词←策略文档→验证老策略"的闭环
...
### 关键词命中率反馈与方向/来源休眠
脚本自动维护 `.keyword_stats.json`，记录每个关键词的搜索数/初筛通过数/最终成功数、所属方向（direction）与来源（source，如 strategy_direct/strategy_derive/explore_ai；explore_suggest/explore_comments 为历史遗留值）：
...
3. **来源休眠提示**：某来源累计 ≥2 个关键词且总产出为 0，休眠提示注入 prompt（开放探索已固定为 AI 衍生，不存在降级链路）
