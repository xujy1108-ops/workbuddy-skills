...
2. **生成关键词** — DeepSeek 按动态配额生成 4 个抖音搜索关键词：①策略直搜（运行时动态拉取内容策略文档，把 S/A 级策略翻译成用户会搜的词，24h 缓存+兜底清单）②策略衍生（抽象策略核心钩子公式，换人群/职业实例化）③开放探索×2（2026-09-16 探索策略重构，多配额块组合）：**配额A 热点翻译**（热点素材表 0-1 跳新鲜热点，跳数门控+同义去重，翻成热点牵出的用钱处境词；无 0-1 跳热点时由坐标系补位）+ **配额B 邻接域反推**（「缺钱但没借钱的人怎么办」反推域：卖黄金/典当/亲友开口等，话题自带缺钱语境，供给新钩子公式）；加强模式第 3 个配额为**处境坐标系**（人群×缺钱时刻格子，借钱需求原生场景，格子由 `.explore_ledger.json` 登记轮转、扫过即销）。探索措辞铁律：处境化（具体的人+具体的难处），禁止议题化（"XX该不该/怎么看/贬值"）。**加强模式**：连续 2 轮零新策略产出（无新建方向且无追加打法）时自动切换为 直搜1 + 探索3、策略衍生暂停，打断"关键词←策略文档→验证老策略"的闭环
...
### 策略文档动态拉取
关键词来源1/2 的策略清单来自飞书内容策略文档（`STRATEGY_DOC_URL`），运行时用 lark-cli 拉取并解析表格（处理 rowspan 合并单元格），只注入 S/A 级策略、整类排除「回应解释」：
- 缓存 24 小时（`.strategy_cache.json`）
- 拉取失败用过期缓存 → 缓存也没有则用脚本内置兜底清单
- 在策略文档里新增方向后，24h 内自动跟上，代码无需修改
