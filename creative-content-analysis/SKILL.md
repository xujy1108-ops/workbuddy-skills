...
2. **生成关键词** — DeepSeek 按三来源结构生成 3 个抖音搜索关键词：①策略直搜（运行时动态拉取内容策略文档，把 S/A 级策略翻译成用户会搜的词，24h 缓存+兜底清单）②策略衍生（抽象策略核心钩子公式，换人群/职业实例化）③开放探索-AI 自由衍生（2026-09-15 起固定走 AI 衍生：按人群处境/金钱摩擦/心理状态/社会现象维度自主造新词，不再用种子词+抖音联想词通道）
...
7. **分析创意** — DeepSeek 先判断内容相关性（强相关/弱相关/不相关）与时效性（长青/时效话题-未过气/时效话题-已过气），再输出内容方向、素材逻辑、借鉴点、植入建议、适配达人分析、内容策略分析（内容方向一/二 + 植入策略，按内容策略表填写指南）；弱相关/不相关、或时效话题已过气（平台功能红利期/阶段性热点已退，如抖音小店先采后付）的内容自动跳过不写入飞书；长青话题（借钱伤感情、求人不如靠自己等）不受发布时间限制，老素材反而沉淀好
...
11. **飞书通知** — 每次运行结束自动给飞书发消息，汇报成功/放弃/失败数量；余额不足时额外提示
